...
![OpenFun 架构设计](docs/diagrams/architecture.svg)

[可编辑的 Excalidraw 架构图](docs/diagrams/architecture.excalidraw)
...
创作者定义 **t0 / epoch0** 时刻的内容、资产和规则，以及后续世界发展的约束。它可以是一个完整的可玩世界。

游玩过程中，玩家行为和世界状态驱动后续更新：生成内容与资产，准备和验证它们，在合适的时机启用，再持久化结果。这些更新形成后续 epoch，可以独立推进并复用未变化的内容，引擎则在更新之间持续运行。

地图可以采用格子、区块、图结构或其他适合游戏的表示。新机制除了资产，还需要兼容的代码和状态变更。已准备的内容与玩家已经体验的内容分别记录；重访和恢复存档时复用已接受的结果。
...
文本与代码模型生成内容、逻辑和生成指令。新的媒体资产通过图片、视频、音频和 3D 生成模型制作，也可以复用合适的已有资产。代码负责逻辑、布局、碰撞与整合。
...
| 阶段               | 目标体验                                                                                                         |
| ------------------ | ---------------------------------------------------------------------------------------------------------------- |
| **2D 游戏**        | 类似泰拉瑞亚的世界，在游玩中生成新地图分区、怪物、道具、资产与剧情，支持经过验证的机制更新，并保留玩家行为的后果 |
| **基础 3D 游戏**   | 类似我的世界，持续生成新地图块、怪物和道具，建立区块加载、空间交互与 3D 资产流程                                 |
| **丰富的开放世界** | 长期向辐射、GTA 等游戏的规模、系统深度和表现力发展                                                               |
