...
| 部分         | 职责                                                               |
| ------------ | ------------------------------------------------------------------ |
| **Context**  | 指导生成的规则、世界描述与游玩记录                                 |
| **Agent**    | 借助文本模型和工具规划任务、生成素材、编写脚本、组合场景并验证更新 |
| **Assets**   | 媒体素材、可执行脚本，以及将它们组合起来的可复用场景               |
| **游戏引擎** | 输入、模拟、游戏状态、物理、渲染，以及启用已准备好的更新           |

### Context：固定规则与持续发展的世界
...
| 部分         | 包含什么                                                 | 如何变化                                           |
| ------------ | -------------------------------------------------------- | -------------------------------------------------- |
| **固定核心** | 核心世界观、基础规则，以及后续生成的边界                 | 由创作者在 epoch0 定义，游玩时的生成不能自行改写   |
| **世界条目** | 对地点、角色、道具、任务、剧情，以及规则允许的机制的描述 | 从 epoch0 开始建立，之后在固定规则内持续补充或更新 |
| **游玩记录** | 已发生的事件、玩家选择，以及已经接受的世界变化           | 随游玩记录，尚未游玩的草稿不算历史                 |

参考[酒馆的世界书（World Info）](https://docs.sillytavern.app/usage/core-concepts/worldinfo/)组织上下文的方式，Agent 每次生成都读取固定核心，并按当前情境选取相关条目和游玩记录。这些持久保存的 Context 为每次生成提供所需知识，再结合引擎提供的当前状态。

Context 描述游戏规则，脚本与校验负责落实。新机制必须遵守固定核心；创作者可以在发布新版本时主动调整这些核心设定。血量、位置等实时状态仍由引擎和存档管理。
...
- **场景：** 将节点、素材、脚本和其他场景组合起来，定义各部分的位置、层级与协作方式，供游戏复用。

在当前 Godot 实现中，[场景由具有层级关系的节点组成](https://docs.godotengine.org/en/stable/getting_started/step_by_step/nodes_and_scenes.html)。脚本可以挂载到节点上，并保存在多个场景共享的外部文件中。贴图、音频、脚本和保存的场景都属于[资源](https://docs.godotengine.org/en/stable/tutorials/scripting/resources.html)，因此统称仍使用 **Assets**，**Nodes** 则表示场景内部的组成单位。

例如，角色的身体、服装、动画、脚步声和移动脚本可以组成角色场景；地形、树木、建筑和角色场景可以组成地图区域；多个区域又可以组成更大的场景。这些组合会保留各部分及其关系，方便复用。每个场景实例的运行状态由引擎管理。

### 初始世界与持续生成

创作者定义 **epoch0 的 Context 与 Assets**，其中包含规则与后续世界发展的约束。它可以是一个完整的可玩世界。

游玩过程中，玩家行为和世界状态为 Agent 的后续更新提供依据。Agent 准备和验证新的 Assets，由引擎在合适的时机启用；已接受的世界变化和发生的事件补充到 Context，并保存结果。这些更新形成后续 epoch，可以独立推进并复用未变化的部分，引擎则在更新之间持续运行。
...
Agent 由文本模型驱动：读取 Context 和引擎状态，调用图片、视频、音频和 3D 模型生成新的媒体素材，并编写游戏脚本。随后通过工具组织素材、脚本、节点和已有场景，组合成可玩的 Assets，验证后交给引擎加载。整个过程中都可以复用合适的已有 Assets。
...
- 发布、发现和再创作 epoch0 版本，包含初始 Context 与 Assets。
- 托管持久化世界和多人会话，共享生成结果。
