...
| 部分         | 职责                                                                      |
| ------------ | ------------------------------------------------------------------------- |
| **内容**     | 固定规则、可扩展的世界条目，以及游玩过程中发生的事情                      |
| **资产**     | 图片、精灵、瓦片、视频、音乐、音效、3D 模型，以及由它们组合成的可复用场景 |
| **游戏引擎** | 输入、模拟、游戏状态、物理、渲染，以及启用已准备好的更新                  |

### Content：固定规则与持续发展的世界

规则属于 Content，但要明确区分哪些必须保持不变，哪些可以持续扩展。

![Content：固定核心、世界条目与游玩记录，为生成提供上下文](docs/diagrams/content.png)

| 部分         | 包含什么                                                       | 如何变化                                           |
| ------------ | -------------------------------------------------------------- | -------------------------------------------------- |
| **固定核心** | 核心世界观、基础规则，以及后续生成的边界                       | 由创作者在 epoch0 定义，游玩时的生成不能自行改写   |
| **世界条目** | 地点、角色、道具、任务、剧情，以及规则允许的机制和对应行为代码 | 从 epoch0 开始建立，之后在固定规则内持续补充或更新 |
| **游玩记录** | 已发生的事件、玩家选择，以及已经接受的世界变化                 | 随游玩记录，尚未游玩的草稿不算历史                 |

参考[酒馆的世界书（World Info）](https://docs.sillytavern.app/usage/core-concepts/worldinfo/)组织上下文的方式，核心信息常驻，其他条目按当前情境选取。OpenFun 的设计是将固定核心、相关世界条目和游玩记录，加上引擎提供的当前状态，组成每次生成所需的上下文。

游戏规则同时通过代码和校验落实。新机制必须遵守固定核心；创作者可以在发布新版本时主动调整这些核心设定。血量、位置等实时状态仍由引擎和存档管理。

### Assets：素材与可复用的场景

Assets 既包括单个媒体素材，也包括由它们组合成的**场景（Scene）**。

![Assets：媒体素材逐层组合成角色、地图区域和更大的场景](docs/diagrams/assets.png)

- **媒体素材：** 图片，包括插画、精灵、贴图、瓦片和动画帧；视频；音乐与音效；以及 3D 模型。
- **场景：** 引用素材和其他场景，组织它们的位置与层级，并关联 Content 中的行为逻辑，形成可复用的组合。

例如，角色的身体、服装、动画和脚步声可以组成角色场景；地形、树木、建筑和角色场景可以组成地图区域；多个区域又可以组成更大的场景，并复用同一批资产。Scene 会保留这些组成部分及其关系，不只是合成后的一张图片。每个场景实例的运行状态由引擎管理。
